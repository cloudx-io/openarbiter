...
// SystemInfo is a snapshot of host resource usage emitted alongside
// ping responses for operational visibility from outside the enclave.
...
// cpuSampleInterval is how long [sampleCPUUsage] waits between
// /proc/stat reads.
const cpuSampleInterval = 100 * time.Millisecond

// GetSystemInfo collects memory and CPU usage from /proc.
...
// parseKeyValueKB reads a /proc/meminfo-style file and returns the
// requested keys' values in kB.
func parseKeyValueKB(path string, keys ...string) (map[string]uint64, error) {
	...
	for scanner.Scan() {
		key, val, ok := parseMemInfoLine(scanner.Text())
		if ok && want[key] {
			result[key] = val
		}
	}
...
func parseCPULine(line string) (*cpuTicks, error) {
	...
	for i, f := range fields[1 : 1+numFields] {
		...
		if err != nil {
			return nil, fmt.Errorf("parse field %d of cpu line: %w", i, err)
		}
		total += v
		if i == 3 {
			idle = v
